...
本文件依實際進行順序保存概念、指令、輸出與判讀；
後續帶練和故障紀錄也接續寫在這裡。
`project/docs/` 的拓撲文件是可交付的架構作品，
只有在查看或更新拓撲與資源假設時需要開啟。
...
| 建立 MUNGE 金鑰並啟動服務 | 讓同一驗證範圍內的請求可驗證身分 | 本機 `a2264` 的憑證驗證成功 |
...
`a2264` 從 `instance-20260923-104239` 提交工作；同一台 VM 上的
`slurmctld` 負責排程、`slurmd` 執行工作、`munged` 提供本機身分驗證，
批次輸出存於 `/home/a2264`。
這台 VM 可見 2 個邏輯 CPU；`slurmd -C` 偵測到 3906 MiB 記憶體，
Slurm 設定為工作宣告 2 個 CPU 與 3000 MiB 記憶體。
目前 `debug` 分區只有這一台節點，沒有獨立 CPU／GPU 節點或共享儲存。

閱讀到這裡，若要查看拓撲圖、各角色的目標部署方式和資源決策表，
請開啟架構成果檔 [cluster-topology.md](../project/docs/cluster-topology.md)；
接著仍沿本文件閱讀資源規劃與下方實作紀錄即可。
