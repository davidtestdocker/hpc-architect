...
用 Vim 寫五行文字，觀察副本內容可獨立修改，使用 `cat` 確認實際內容；學員決定在此收尾。
...
副本已與原始檔不同，但 `modified copy.` 與原本第四行的 `Copies...` 被接為同一行，表示兩段文字之間少了換行。此差異保留作為編輯器會精確保留字元與換行的實例；本日依學員決定在此收尾。
...
本日練習結束。已提供刪除兩個練習檔的清理指令；不將未安排的延伸操作列為完成條件。
